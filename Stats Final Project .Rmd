...
```{r}
cancer.pred.lm <- lm(CANCER_CrudePrev ~ . , data = df.unhealthy.precitors)
summary(cancer.pred.lm)
```

```{r}
cancer.pred.reduced <- lm(CANCER_CrudePrev ~ ACCESS2_CrudePrev + BINGE_CrudePrev  + BPMED_CrudePrev + CHECKUP_CrudePrev + CHOLSCREEN_CrudePrev + COLON_SCREEN_CrudePrev + COREM_CrudePrev + COREW_CrudePrev  + CSMOKING_CrudePrev + DENTAL_CrudePrev + OBESITY_CrudePrev + PAPTEST_CrudePrev + SLEEP_CrudePrev + TEETHLOST_CrudePrev, data = df.unhealthy.precitors)
summary(cancer.pred.reduced)
anova(cancer.pred.lm, cancer.pred.reduced)
```
