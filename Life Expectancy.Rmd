...
summary(develop_stepmod_intred_attempt2)
bptest(develop_stepmod_intred_attempt2)
...
bptest(developing_intred2)
plot(developing_intred2)
...
FROM ASSUMPTION CHECK WE DO BOXPLOT
...
ATTEMPTING THE SAME ASSUMPTIONS AND HIGHER ORDER WITH MODEL MADE FROM STEP() FUNCTION


```{r}
develop_stepmod_intred_attempt2 = step(develop_lmintfull)
summary(develop_stepmod_intred_attempt2)
```

```{r}
#ggpairs(develop_stepmod_intred_attempt2)


develop_stepmod_intred_attempt2HIGHER = lm(Life.expectancy ~ Alcohol + Hepatitis.B  + BMI + Polio + Diphtheria + HIV.AIDS + I(HIV.AIDS^2) + I(HIV.AIDS^3) + I(HIV.AIDS^4) + Alcohol:Hepatitis.B + Alcohol:BMI + Alcohol:Diphtheria + Alcohol:HIV.AIDS + Hepatitis.B:BMI + Hepatitis.B:Diphtheria + BMI:Polio + BMI:Diphtheria + Polio:Diphtheria + Diphtheria:HIV.AIDS, data = developing_LE)

summary(develop_stepmod_intred_attempt2HIGHER)
bptest(develop_stepmod_intred_attempt2HIGHER)
plot(develop_stepmod_intred_attempt2HIGHER)

shapiro.test(residuals(develop_stepmod_intred_attempt2HIGHER))
```

BOXCOX TEST
```{r}
bc = boxcox(develop_stepmod_intred_attempt2HIGHER, lambda=seq(-1,1))
...
lev=hatvalues(develop_stepmod_intred_attempt2)
p = length(coef(develop_stepmod_intred_attempt2))
...
higher3plm = lm(Life.expectancy ~ Alcohol + Hepatitis.B + BMI + Polio + Diphtheria + HIV.AIDS + I(HIV.AIDS^2) + + I(HIV.AIDS^3) + I(HIV.AIDS^4) + I(HIV.AIDS^5) + Alcohol:Hepatitis.B + Alcohol:BMI + Alcohol:Diphtheria + Alcohol:HIV.AIDS + Hepatitis.B:Diphtheria + Hepatitis.B:HIV.AIDS + BMI:Polio + BMI:Diphtheria + Diphtheria:HIV.AIDS, data = developing_LE_no_outliersSTEP)

#anova accepts higher order model
anova(higher3plm,intred3plm)
...
```{r}
developing_highermodNOOUTLIER = lm(Life.expectancy ~ HIV.AIDS + I(HIV.AIDS^2) + I(HIV.AIDS^3) + I(HIV.AIDS^4) + Hepatitis.B + BMI + Alcohol + Polio + Diphtheria + Polio:Diphtheria +BMI:Polio + Hepatitis.B:Alcohol + HIV.AIDS:Alcohol + Diphtheria:Alcohol +  Hepatitis.B:Polio + HIV.AIDS:Diphtheria, data = developing_LE_no_outliersSTEP)
summary(developing_highermodNOOUTLIER)

bptest(developing_highermodNOOUTLIER)
plot(developing_highermodNOOUTLIER)

#still gives 1
#boxcox
#bc = boxcox(developing_highermodNOOUTLIER,lambda=seq(-1,1))
#bestlambda=bc$x[which(bc$y==max(bc$y))]
#bestlambda
...
#reduced model
red2plm = lm(Life.expectancy ~  Alcohol + Hepatitis.B + Measles + BMI + Diphtheria + HIV.AIDS, data = developing_LE_no_outliersSTEP2P)
summary(red2plm)

#int model
int2plm = lm(Life.expectancy ~  (Alcohol + Hepatitis.B + Measles + BMI + Diphtheria + HIV.AIDS)^2, data = developing_LE_no_outliersSTEP2P)
summary(int2plm)

intred2plm = step(int2plm)
summary(intred2plm)
